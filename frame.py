    ...
    def __init__(self, img, camera, pose=None, id=None, timestamp=None, kps_data=None):
        ...
        if img is not None:
            ...
            self.kpsu = self.camera.undistort_points(
                self.kps)  # convert to undistorted keypoint coordinates
    ...
    def draw_feature_trails(self, img, kps_idxs, trail_max_length=9):
        ...
        with self._lock_features:
            uvs = np.rint(self.kps[kps_idxs]).astype(
                np.intp)  # image keypoints coordinates  # use distorted coordinates when drawing on distorted original image
            ...
            for kp_idx in kps_idxs:
                ...
                if point is not None and not point.is_bad:
                    ...
                    if len(p_frame_views) > 2:
                        cv2.circle(img, uv, color=(0, 255, 0), radius=radius,
                                   thickness=1)  # draw keypoint size as a circle
                        # cv2.circle(img, uv, color=color, radius=radius)
                    else:
                        cv2.circle(img, uv, color=(255, 0, 0), radius=radius,
                                   thickness=1)  # draw keypoint size as a circle
